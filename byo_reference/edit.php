...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if (isset($_POST['action']) && $_POST['action'] === 'delete') {
    ...
    echo '削除しました。<a href="index.html">トップへ戻る</a>';
    exit;
  }
  ...
  echo '更新しました。<a href="index.html">トップへ戻る</a>';
  exit;
}
...
<body>
  ...
  <main class="main-container">
    ...
    <form action="edit.php" method="post" class="form-byo">
      <input type="hidden" name="id" value="<?= htmlspecialchars($entry['id'], ENT_QUOTES, 'UTF-8') ?>">

      <div class="form-row">
        <label>お名前：
          <input type="text" name="user_name" value="<?= htmlspecialchars($entry['user_name'], ENT_QUOTES, 'UTF-8') ?>">
        </label>
      </div>
      ...
      <div class="form-row">
        <label>ワイン名：
          <input type="text" name="wine_name" value="<?= htmlspecialchars($entry['wine_name'], ENT_QUOTES, 'UTF-8') ?>">
        </label>
      </div>
      ...
      <div class="form-row">
        <label>タイプ：
          <input type="text" name="wine_type" value="<?= htmlspecialchars($entry['wine_type'], ENT_QUOTES, 'UTF-8') ?>">
        </label>
      </div>
